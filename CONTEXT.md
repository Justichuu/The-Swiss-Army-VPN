...
Source is `1.5.1.0`. The installer refuses to replace the same version, so a new package needs a new fourth number. Tags look like `v1.5.1.0`.

Bring-your-own connect is in this version: Swiss NordVPN, Any NordVPN, or a safe IKEv2 hostname / IPv4 you bring. Fresh installs still start on the packaged Swiss server. WireGuard and OpenVPN are not driven by CONNECT.
